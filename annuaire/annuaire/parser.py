    ...
    for i, item in enumerate(src.xpath("//table[@class='shadow-details-2']/tr/td")):
        if not item.text or not item.text.strip():
            if len(item.xpath(".//a")) == 1:
                ...
                m = re.search(r"identifiantAvocat=(\d+)", item.xpath(".//a")[0].attrib["href"])
                tmp["id"] = m.group(1)
        ...
        m = re.search(r"([A-Z\s]+)\s,\s*(\d*\s.*)\s-\s(\d+)\s([\w\s]+)", item.text.strip())
        if m:
...
def parse_detail(src):
    ...
    tmp = "@@@@".join(res)
    tmp = tmp.replace(":@@@@", ": ")
    m = re.search(r"Date de prestation de serment :\s+(\d+\s*\/\s+\d+\s*\/\s+\d+)\s*@@@@", tmp)
    m1 = re.search(r"Mention\(s\) de spécialisation :\s*(.*)\s*@@@@", tmp)
    m2 = re.search(r"Langue\(s\) :\s*(\w+)\s*", tmp)
    return {
